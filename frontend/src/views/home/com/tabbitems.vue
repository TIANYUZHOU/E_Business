<template>
  <div id="produceItem">
    <div class="item" v-for="(product, index) in product_lists" :key="index">
      <!-- 绑定key起缓存作用 避免刷新丢失已加载的数据 -->
      <img v-lazy="product.small_image" :key="product.small_image"/>
      <p class="itemTitle">{{ product.name }}</p>
      <!--<span class="tagEat" v-if="isShowEatTag">吃货节</span>-->
      <span id="mytext1" class="price">{{ product.price | moneyFormat }}</span>
      <span class="originPrice">{{ product.origin_price | moneyFormat }}</span>
      <div class="buyCar" @click="addToCart(product)">
